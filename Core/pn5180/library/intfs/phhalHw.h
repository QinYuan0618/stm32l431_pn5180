...
#define PHHAL_HW_PN5190_RATS_LEN                    0x02U       /**< Buffer size to copy RATS, used in case of LPCD with HCE command handling. */
#define PHHAL_HW_PN5190_RATS_CMD                    0xE0U       /**< RATS Command (1st Byte). */
/**
 ...
 */
    typedef struct
    {
        ...
        uint32_t dwExpectedEvent;                           /**< ExpectedEvent is used by Event Wait, to process command specific events. */

        uint8_t bFeliCaRxOnly;                              /**< Flag indicating if Exchange needs to perform only Rx operation when configured as FeliCa Reader. */

        uint8_t bAbortCMD;                                  /**< Flag indicating which SWITCH MODE NORMAL Command needs to be sent to abort an ongoing CMD execution. */

        uint8_t bTimerDeviation;                            /**< Platform timer deviation percentage. */

        uint8_t bLpcdHceModeRats[PHHAL_HW_PN5190_RATS_LEN]; /**< Buffer to copy RATS, used in case of LPCD with HCE command. */

        uint8_t bLpcdHceRatsFlag;                           /**< Flag indicates RATS received state in case of LPCD with HCE command. */
