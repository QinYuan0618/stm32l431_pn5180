                                                      ...
                                                      )
{
    ...
    if((status == PH_ERR_PN5190_SUCCESS) || (status == PH_ERR_PN5190_INSTR_ERROR))
    {
        if((pRxData != NULL) && (wRxLen == PHHAL_HW_PN5190_INSTR_GET_CRC_USER_AREA_RESP_LEN))
        {
            /* Copy EEPROM user configuration area CRC and protocol area CRC to user buffer */
            memcpy(pValue, pRxData, wRxLen);
        }
    }
    return PH_ADD_COMPCODE(status, PH_COMP_HAL);
}
                                               ...
                                               )
{
    ...
    case PHHAL_HW_CONFIG_LPCD_CONFIG:
        if ((dwValue <=  PHHAL_HW_PN5190_LPCD_CTRL_SINGLEMODE_LPCD) ||
            ...
            (dwValue == PHHAL_HW_PN5190_LPCD_CTRL_LPCD_HCE))
        {
                                              ...
                                              )
{
    ...
    /* Command Type */
    sCmdParams.bCmd = PHHAL_HW_PN5190_INSTR_FAST_CTS_CONFIG;
