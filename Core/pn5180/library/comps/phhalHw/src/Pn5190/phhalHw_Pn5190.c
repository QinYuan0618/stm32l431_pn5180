                               ...
                               )
{
    ...
    pDataParams->bLpcdHceRatsFlag       = PH_OFF;
                               ...
                               )
{
    ...
    uint8_t  PH_MEMLOC_REM * ppRxBuffer = NULL;
    uint16_t PH_MEMLOC_REM wRxLength = 0x0U;
       ...
       (pDataParams->bLpcdMode == PHHAL_HW_PN5190_LPCD_MODE_POWERDOWN))
    {
        ...
        if (pDataParams->bLpcdMode == PHHAL_HW_PN5190_LPCD_MODE_DEFAULT )
        {
            dwEventToWait = PH_PN5190_EVT_LP_CALIB | PH_PN5190_EVT_GENERAL_ERROR | PH_PN5190_EVT_AUTOCOLL;
        }
        else
        {
            dwEventToWait = PH_PN5190_EVT_LPCD | PH_PN5190_EVT_GENERAL_ERROR | PH_PN5190_EVT_AUTOCOLL;
        }
        ...
        if(status == PH_ERR_SUCCESS)
        {
            ...
            if(status == PH_ERR_SUCCESS)
            {
                ...
                else if(dwEventReceived == PH_PN5190_EVT_AUTOCOLL) /* Autocoll Event received. */
                {
                    if ((pDataParams->dwEventStatus & 0xFF) == PHHAL_HW_PN5190_AUTOCOLL_ACTIVATED) /* Autocoll Activated*/
                    {
                        /* Retrieve Data (RATS) */
                        status = phhalHw_Pn5190_Instr_RetrieveRFData(pDataParams, &ppRxBuffer, &wRxLength);
                        if (wRxLength == (PHHAL_HW_PN5190_RATS_LEN + 2U)) /* RATS including CRC */
                        {
                            /* Copy RATS to HAL data param */
                            memcpy(&pDataParams->bLpcdHceModeRats, ppRxBuffer, PHHAL_HW_PN5190_RATS_LEN);
                            /* Check card mode Type A, 106kbps activated and validate RATS command (1st Byte). */
                            if(((pDataParams->dwEventStatus & 0xFFFFFF) == 0x00011000)
                                && (pDataParams->bLpcdHceModeRats[0] == PHHAL_HW_PN5190_RATS_CMD))
                            {
                                /* RATS received successfully. */
                                pDataParams->bLpcdHceRatsFlag = PH_ON;
                            }
                        }
                    }
                }
