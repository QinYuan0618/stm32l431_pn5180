    ...
    uint8_t ** ppResponse, uint16_t * pRxlen)
{
    ...
    if (bWrappedMode)
    {
        ...
        pApdu[4] = (((wCmdLen)-0x01u) & 0xFF);

        PH_CHECK_SUCCESS_FCT(statusTmp, phpalMifare_ExchangeL4(
            pPalMifareDataParams,
            (uint16_t)(pApdu[4] == 0x00 ? PH_EXCHANGE_DEFAULT : PH_EXCHANGE_BUFFER_FIRST),
            pApdu,
    ...
    uint32_t bLc, uint8_t bExtendedLenApdu, uint8_t * pDataIn, uint32_t bLe, uint8_t ** ppDataOut, uint16_t *pDataLen)
{
    ...
    case PHAL_MFDFLIGHT_CMD_ISO7816_SELECT_FILE:
        ...
        statusTmp = phalMfdfLight_Int_ISOSelectFile(pDataParams, pPalMifareDataParams, bOption, bCmdBuff, wCmdLen,
            (uint8_t)(bLc & 0xFF), bExtendedLenApdu, pDataIn, (uint8_t)(bLe & 0xFF), ppDataOut, pDataLen);
        break;
