...
phStatus_t phKeyStore_SetKeyASym(void * pDataParams, uint16_t wKeyNo, uint16_t wPos, uint16_t wKeyType, uint16_t wKeyInfo, uint8_t * pKey,
    uint16_t wKeyLen)
{
    phStatus_t PH_MEMLOC_REM status;

    PH_LOG_HELPER_ALLOCATE_TEXT(bFunctionName, "phKeyStore_SetKeyASym");
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(wKeyNo);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(wPos);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(wKeyType);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(wKeyInfo);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(pKey);
    ...
    PH_LOG_HELPER_ADDPARAM_UINT16(PH_LOG_LOGTYPE_DEBUG, PH_LOG_VAR(wPos), &wPos);
    PH_LOG_HELPER_ADDPARAM_UINT16(PH_LOG_LOGTYPE_DEBUG, PH_LOG_VAR(wKeyType), &wKeyType);
    PH_LOG_HELPER_ADDPARAM_UINT16(PH_LOG_LOGTYPE_DEBUG, PH_LOG_VAR(wKeyInfo), &wKeyInfo);
...
#ifdef NXPBUILD__PH_KEYSTORE_SW
        case PH_KEYSTORE_SW_ID:
            status = phKeyStore_Sw_SetKeyASym((phKeyStore_Sw_DataParams_t *) pDataParams, wKeyNo, wPos, wKeyType, wKeyInfo, pKey, wKeyLen);
            break;
#endif /* NXPBUILD__PH_KEYSTORE_SW */
...
phStatus_t phKeyStore_GetKeyASym(void * pDataParams, uint16_t wKeyNo, uint16_t wPos, uint16_t wKeyPairType, uint16_t * pKeyType,
    uint8_t * pCurveID, uint8_t * pKey, uint16_t * pKeyLen)
{
    phStatus_t PH_MEMLOC_REM status;

    PH_LOG_HELPER_ALLOCATE_TEXT(bFunctionName, "phKeyStore_GetKeyASym");
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(wKeyNo);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(wPos);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(wKeyPairType);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(pKeyType);
    ...
    PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_KEYSTORE);
    PH_ASSERT_NULL_PARAM(pKeyType, PH_COMP_KEYSTORE);
    PH_ASSERT_NULL_PARAM(pCurveID, PH_COMP_KEYSTORE);
    PH_ASSERT_NULL_PARAM(pKey, PH_COMP_KEYSTORE);
    PH_ASSERT_NULL_PARAM(pKeyLen, PH_COMP_KEYSTORE);

    /* Log the information. */
    PH_LOG_HELPER_ADDSTRING(PH_LOG_LOGTYPE_INFO, bFunctionName);
    PH_LOG_HELPER_ADDPARAM_UINT16(PH_LOG_LOGTYPE_DEBUG, PH_LOG_VAR(wKeyNo), &wKeyNo);
    PH_LOG_HELPER_ADDPARAM_UINT16(PH_LOG_LOGTYPE_DEBUG, PH_LOG_VAR(wPos), &wPos);
    PH_LOG_HELPER_ADDPARAM_UINT16(PH_LOG_LOGTYPE_DEBUG, PH_LOG_VAR(wKeyPairType), &wKeyPairType);
    PH_LOG_HELPER_EXECUTE(PH_LOG_OPTION_CATEGORY_ENTER);
...
#ifdef NXPBUILD__PH_KEYSTORE_SW
        case PH_KEYSTORE_SW_ID:
            status = phKeyStore_Sw_GetKeyASym((phKeyStore_Sw_DataParams_t *) pDataParams, wKeyNo, wPos, wKeyPairType, pKeyType, pCurveID,
                pKey, pKeyLen);
            break;
#endif /* NXPBUILD__PH_KEYSTORE_SW */
...
phStatus_t phKeyStore_GetCurveID(void * pDataParams, uint16_t wKeyNo, uint16_t wPos, uint8_t * pCurveID)
{
    ...
    PH_LOG_HELPER_ALLOCATE_TEXT(bFunctionName, "phKeyStore_GetCurveID");
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(wKeyNo);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(wPos);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(pCurveID);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(status);
...
#ifdef NXPBUILD__PH_KEYSTORE_SW
        case PH_KEYSTORE_SW_ID:
            status = phKeyStore_Sw_GetCurveID((phKeyStore_Sw_DataParams_t *) pDataParams, wKeyNo, wPos, pCurveID);
            break;
#endif /* NXPBUILD__PH_KEYSTORE_SW */
