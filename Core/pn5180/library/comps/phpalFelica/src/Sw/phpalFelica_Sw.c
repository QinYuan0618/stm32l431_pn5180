    ...
    *pMoreCardsAvailable = 0;
    ...
    if (0U != (bIDmPMmLength))
    {
        ...
        if ((bIDmPMmLength != (PHPAL_FELICA_IDM_LENGTH + PHPAL_FELICA_PMM_LENGTH)) &&
            (bIDmPMmLength != (PHPAL_FELICA_IDM_LENGTH + PHPAL_FELICA_PMM_LENGTH + PHPAL_FELICA_RD_LENGTH)))
        {
            return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_PAL_FELICA);
        }
        ...
        {
            /* Reset RD data */
            (void)memset((uint8_t *)&(pDataParams->aIDmPMm[PHPAL_FELICA_IDM_LENGTH + PHPAL_FELICA_PMM_LENGTH]), 0x00, PHPAL_FELICA_RD_LENGTH);
        }
        /* Store IDm, PMm and RD(if available) */
    ...
    do{
        ...
        if(bNumTimeSlots > PHPAL_FELICA_NUMSLOTS_1)
        {
            ...
            while(0U != bTemp)
            {
                *pRxLength = 0x00;
                ...
                {
                    (void)memcpy(pRxBuffer, pId, *pRxLength);
                    (void)memcpy(pDataParams->aIDmPMm, pId, *pRxLength);
                    pDataParams->bIDmPMmValid = PHPAL_FELICA_SW_IDMPMM_VALID;
                    return status;
                }
                bTemp--;
        ...
        else
        {
            ...
            {
                *pRxLength = (uint8_t)(wRxLen & 0x00FFU);
                (void)memcpy(pRxBuffer, pRxBuff, *pRxLength);
                (void)memcpy(pDataParams->aIDmPMm, pRxBuff, *pRxLength);
                pDataParams->bIDmPMmValid = PHPAL_FELICA_SW_IDMPMM_VALID;
                return status;
            }
        }
