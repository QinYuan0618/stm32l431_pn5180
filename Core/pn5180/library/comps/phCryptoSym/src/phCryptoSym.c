/*----------------------------------------------------------------------------*/
/* Copyright 2009 - 2019, 2022, 2024 - 2025 NXP                               */
/*                                                                            */
...
phStatus_t phCryptoSym_Decrypt(void * pDataParams, uint16_t wOption, uint8_t * pEncryptedBuffer, uint16_t wBufferLength,
...
phStatus_t phCryptoSym_LoadNonce(void * pDataParams, uint8_t bCipher, const uint8_t * pNonce, uint8_t bNonceLen)
{
    ...
    PH_LOG_HELPER_ALLOCATE_TEXT(bFunctionName, "phCryptoSym_LoadNonce");
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(bCipher);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(pNonce);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(bNonceLen);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(status);

    /* Validate the parameters. */
    PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_CRYPTOSYM);
    PH_ASSERT_NULL_PARAM(pNonce, PH_COMP_CRYPTOSYM);

    /* Log the information. */
    PH_LOG_HELPER_ADDSTRING(PH_LOG_LOGTYPE_INFO, bFunctionName);
    PH_LOG_HELPER_ADDPARAM_UINT8(PH_LOG_LOGTYPE_DEBUG, PH_LOG_VAR(bCipher), &bCipher);
    PH_LOG_HELPER_ADDPARAM_BUFFER(PH_LOG_LOGTYPE_DEBUG, PH_LOG_VAR(pNonce), pNonce, bNonceLen);
    PH_LOG_HELPER_ADDPARAM_UINT8(PH_LOG_LOGTYPE_DEBUG, PH_LOG_VAR(bNonceLen), &bNonceLen);
    PH_LOG_HELPER_EXECUTE(PH_LOG_OPTION_CATEGORY_ENTER);
...
#ifdef NXPBUILD__PH_CRYPTOSYM_MBEDTLS
        case PH_CRYPTOSYM_MBEDTLS_ID:
            status = phCryptoSym_mBedTLS_LoadNonce((phCryptoSym_mBedTLS_DataParams_t *) pDataParams, bCipher, pNonce, bNonceLen);
            break;
#endif /* NXPBUILD__PH_CRYPTOSYM_MBEDTLS */
...
phStatus_t phCryptoSym_LoadAdditionalData(void * pDataParams, const uint8_t * pAddData, uint16_t wAddData_Len)
{
    ...
    PH_LOG_HELPER_ALLOCATE_TEXT(bFunctionName, "phCryptoSym_LoadAdditionalData");
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(pAddData);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(wAddData_Len);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(status);

    /* Validate the parameters. */
    PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_CRYPTOSYM);
    PH_ASSERT_NULL_PARAM(pAddData, PH_COMP_CRYPTOSYM);
    if(wAddData_Len >PH_CRYPTOSYM_AES_CCM_ADD_DATA_SIZE)
        return PH_ADD_COMPCODE_FIXED(PH_ERR_PARAMETER_SIZE, PH_COMP_CRYPTOSYM);

    /* Log the information. */
    PH_LOG_HELPER_ADDSTRING(PH_LOG_LOGTYPE_INFO, bFunctionName);
    PH_LOG_HELPER_ADDPARAM_BUFFER(PH_LOG_LOGTYPE_DEBUG, PH_LOG_VAR(pAddData), pAddData, wAddData_Len);
    PH_LOG_HELPER_ADDPARAM_UINT16(PH_LOG_LOGTYPE_DEBUG, PH_LOG_VAR(wAddData_Len), &wAddData_Len);
    PH_LOG_HELPER_EXECUTE(PH_LOG_OPTION_CATEGORY_ENTER);
...
#ifdef NXPBUILD__PH_CRYPTOSYM_MBEDTLS
        case PH_CRYPTOSYM_MBEDTLS_ID:
            status = phCryptoSym_mBedTLS_LoadAdditionalData((phCryptoSym_mBedTLS_DataParams_t *) pDataParams, pAddData, wAddData_Len);
            break;
#endif /* NXPBUILD__PH_CRYPTOSYM_MBEDTLS */
...
phStatus_t phCryptoSym_SetAuthenticationTag(void * pDataParams, const uint8_t * pTag, uint8_t bTag_Len)
{
    ...
    PH_LOG_HELPER_ALLOCATE_TEXT(bFunctionName, "phCryptoSym_SetAuthenticationTag");
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(pTag);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(bTag_Len);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(status);

    /* Validate the parameters. */
    PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_CRYPTOSYM);
    PH_ASSERT_NULL_PARAM(pTag, PH_COMP_CRYPTOSYM);

    /* Log the information. */
    PH_LOG_HELPER_ADDSTRING(PH_LOG_LOGTYPE_INFO, bFunctionName);
    PH_LOG_HELPER_ADDPARAM_BUFFER(PH_LOG_LOGTYPE_DEBUG, PH_LOG_VAR(pTag), pTag, bTag_Len);
    PH_LOG_HELPER_ADDPARAM_UINT8(PH_LOG_LOGTYPE_DEBUG, PH_LOG_VAR(bTag_Len), &bTag_Len);
    PH_LOG_HELPER_EXECUTE(PH_LOG_OPTION_CATEGORY_ENTER);
...
#ifdef NXPBUILD__PH_CRYPTOSYM_MBEDTLS
        case PH_CRYPTOSYM_MBEDTLS_ID:
            status = phCryptoSym_mBedTLS_SetAuthenticationTag((phCryptoSym_mBedTLS_DataParams_t *) pDataParams, pTag, bTag_Len);
            break;
#endif /* NXPBUILD__PH_CRYPTOSYM_MBEDTLS */
...
phStatus_t phCryptoSym_GetAuthenticationTag(void * pDataParams, uint8_t * pTag, uint8_t * pTag_Len)
{
    ...
    PH_LOG_HELPER_ALLOCATE_TEXT(bFunctionName, "phCryptoSym_GetAuthenticationTag");
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(pTag);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(pTag_Len);
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(status);

    /* Validate the parameters. */
    PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_CRYPTOSYM);
    PH_ASSERT_NULL_PARAM(pTag, PH_COMP_CRYPTOSYM);
    PH_ASSERT_NULL_PARAM(pTag_Len, PH_COMP_CRYPTOSYM);
...
#ifdef NXPBUILD__PH_CRYPTOSYM_MBEDTLS
        case PH_CRYPTOSYM_MBEDTLS_ID:
            status = phCryptoSym_mBedTLS_GetAuthenticationTag((phCryptoSym_mBedTLS_DataParams_t *) pDataParams, pTag, pTag_Len);
            break;
#endif /* NXPBUILD__PH_CRYPTOSYM_MBEDTLS */
    ...
    if((status & PH_ERR_MASK) == PH_ERR_SUCCESS)
    {
    PH_LOG_HELPER_ADDPARAM_BUFFER(PH_LOG_LOGTYPE_DEBUG, PH_LOG_VAR(pTag), pTag, *pTag_Len);
    PH_LOG_HELPER_ADDPARAM_UINT8(PH_LOG_LOGTYPE_DEBUG, PH_LOG_VAR(pTag_Len), pTag_Len);
    }
#endif /* NXPBUILD__PH_LOG */
    PH_LOG_HELPER_ADDPARAM_UINT16(PH_LOG_LOGTYPE_INFO, PH_LOG_VAR(status), &status);
    PH_LOG_HELPER_EXECUTE(PH_LOG_OPTION_CATEGORY_LEAVE);
