    ...
    *pResponse = pRecv;
    ...
    if (bStatusByte == PHAL_MFNTAG42XDNA_RESP_ADDITIONAL_FRAME)
    {
        if (*pRxlen == 2U)
        {
         ...
         /* Next position will ensure overwriting on the
            ...
            wNextPos = (*pRxlen) - 2U;
            memcpy(bBackupBytes, &(*pResponse)[wNextPos - 3U], 3); /* PRQA S 3200 */
    ...
    while (bStatusByte == PHAL_MFNTAG42XDNA_RESP_ADDITIONAL_FRAME)
    {
        ...
        else
        {
            ...
            memcpy(&(*pResponse)[wNextPos - 3U], bBackupBytes, 3); /* PRQA S 3200 */
    ...
    uint16_t wCmdLen, uint8_t ** ppRxdata, uint16_t * pRxdataLen)
{
        ...
        ((bOption & 0xF0U) == PHAL_MFNTAG42XDNA_COMMUNICATION_MACD))
    {
        ...
        if ((status & PH_ERR_MASK) == PH_ERR_SUCCESS)
        {
            memcpy(bCMacCard, &pRecv[wRxlen - 8U], 8); /* PRQA S 3200 */
            wRxlen -= 8U;
...

{
    ...
    if (pDataParams->bAuthMode == PHAL_MFNTAG42XDNA_AUTHENTICATEEV2)
    {
        ...
        if (wDataLen1)
        {
            if((wTmp < 32U) && ((bCmdBuff[0] != PHAL_MFNTAG42XDNA_RESP_ADDITIONAL_FRAME)))
            {
                wCommMode = PH_EXCHANGE_BUFFER_FIRST;
            }
    ...
    */
    ...
    {
        wApduLen = ((wCmdLen == 0x01U ) && (bIns == PHAL_MFNTAG42XDNA_ISO_CHAINING_MODE ))? 0 : PHAL_MFNTAG42XDNA_WRAP_HDR_LEN;
        wCmdLen = ((wCmdLen == 0x01U ) && (bIns == PHAL_MFNTAG42XDNA_ISO_CHAINING_MODE ))? 0 : wCmdLen;
       ...
       pApdu[1] = bCmdBuff[0]; /* Desfire cmd code in INS */
        if(wCmdLen > 0)
        {
            ...
            PH_CHECK_SUCCESS_FCT(statusTmp, phpalMifare_ExchangeL4(
                ...
                (((bIns == PHAL_MFNTAG42XDNA_ISO_CHAINING_MODE) && (bCommOption & PHAL_MFNTAG42XDNA_MAC_DATA_INCOMPLETE)) ? PH_EXCHANGE_TXCHAINING : 0),
                &pApdu[2],
                (uint16_t)((pDataParams->dwPayLoadLen > 0xFEU ) && (bIns == PHAL_MFNTAG42XDNA_ISO_CHAINING_MODE ))? 0x02U : 0x01U,
                &pRecv,
                &wRxlen));
    ...
    if (pDataParams->bAuthMode == PHAL_MFNTAG42XDNA_AUTHENTICATEEV2)
    {
        ...
        memcpy(bCMacCard, &pResp[wRxlen - 8U], 8);  /* PRQA S 3200 */
        wRxlen -= 8U;
...
#endif /* NXPBUILD__PH_CRYPTOSYM */
                /* Copy the remaining bCmdBuff into bWorkBuffer */
                memcpy(bWorkBuffer, &bCmdBuff[(32U - wWorkBufferLen) + 1], (wTmp - 32U)); /* PRQA S 3200 */
                wWorkBufferLen = (wTmp - 32U);
            }
            else
            {
    ...
    if (pDataParams->bAuthMode == PHAL_MFNTAG42XDNA_AUTHENTICATEEV2)
    {
        if  ((status & PH_ERR_MASK) != PH_ERR_SUCCESS_CHAINING) /* if no chaining ongoing */
        {
            ...
            if ((bCommOption & 0xF0U) == PHAL_MFNTAG42XDNA_COMMUNICATION_MACD)
            {
                ...
                {
                    /* copy CMAC received from card*/
                    memcpy(bCMacCard, &bWorkBuffer[wRxlen - 8U], 8);  /* PRQA S 3200 */
                    wRxlen -= 8U;
    ...
    uint8_t * pResp, uint16_t * pRespLen)
{
    ...
    else
    {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phalMfNtag42XDna_Sw_Int_GetFrameLength(
            ...
            wApduHeaderLen = PHAL_MFNTAG42XDNA_WRAP_HDR_LEN + (bExtended7816 ? 2U : 0);
    ...
    do
    {
        ...
        wLeFieldLen = ((bIns == PHAL_MFNTAG42XDNA_ISO_CHAINING_MODE) && (pDataParams->bWrappedMode == 1U)) ? 1U : 0;
        ...
        bCommOptionTmp = (uint8_t)(((!wDataToBeSent) && ( ! bCommOption )) ? 0 : 1U);
        ...
        if (pDataParams->bWrappedMode)
        {
            ...
            if(bExtended7816 && (bIns == PHAL_MFNTAG42XDNA_ISO_CHAINING_MODE))
            {
                ...
                pApdu[4] = (uint8_t)((pDataParams->dwPayLoadLen >> 16)& 0xFFU);
                pApdu[5] = (uint8_t)((pDataParams->dwPayLoadLen >> 8)& 0xFFU);
        ...
        {
            bStatusByte = (uint8_t)((pRecv[0] & 0xF0U) == 0xA0U) ? PHAL_MFNTAG42XDNA_RESP_ADDITIONAL_FRAME : PH_ERR_PROTOCOL_ERROR;
        }
        else
        {
        ...
        if(bStatusByte != 0x00)
        {
            ...
            wCmdLen = (bIns == PHAL_MFNTAG42XDNA_ISO_CHAINING_MODE)? 0: 1U;
    ...
    uint16_t wCmdLen, uint8_t ** ppRxBuffer, uint32_t * pBytesRead)
{
    ...
    while ((status & PH_ERR_MASK) == PH_ERR_SUCCESS_CHAINING)
    {
        ...
        wNextPos = *pBytesRead;
        memcpy(bBackupBytes, &pRecv[wNextPos - 3U], 3); /* PRQA S 3200 */
        ...
        memcpy(&pRecv[wNextPos - 3U], bBackupBytes, 3); /* PRQA S 3200 */
...
/* Validate parameters
   ...
   /* Validate Card Key Number */
    ...
    if ((wOption != PHAL_MFNTAG42XDNA_NO_DIVERSIFICATION) && (bDivLen > 31U))
    {
        return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_AL_MFNTAG42XDNA);
    }
    if(bLenPcdCapsIn > 6U)
    ...
    memcpy(&bCmdBuff[bRndLen + 1], &bRndB[1], bRndLen - 1U); /* PRQA S 3200 */
    ...
    uint16_t wKeyVer, uint8_t *pSDMReadCtr, uint8_t * pVCUID, uint8_t bUidLen, uint8_t *pSessEncKey, uint8_t *pSessMacKey)
{
    ...
    bSV2[bCmdLenMac++] = bSV1[bCmdLenEnc++] = 0x00U;
    bSV2[bCmdLenMac++] = bSV1[bCmdLenEnc++] = 0x01U;
    bSV2[bCmdLenMac++] = bSV1[bCmdLenEnc++] = 0x00U;
    bSV2[bCmdLenMac++] = bSV1[bCmdLenEnc++] = 0x80U;
