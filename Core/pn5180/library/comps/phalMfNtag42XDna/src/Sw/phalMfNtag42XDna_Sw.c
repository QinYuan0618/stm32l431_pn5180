    ...
    uint8_t * pOffset, uint8_t * pData, uint8_t * pDataLen)
{
    ...
    */
    ...
    else
    {
        ...
        {
            pDataParams->dwPayLoadLen = pDataParams->dwPayLoadLen + ((pDataParams->bAuthMode == PHAL_MFNTAG42XDNA_AUTHENTICATE) ? 0x04U : 0x08U);
        }
        if((bCommOption & 0xF0U) == PHAL_MFNTAG42XDNA_COMMUNICATION_ENC)
        {
            ...
            {
                pDataParams->dwPayLoadLen  = pDataParams->dwPayLoadLen  + ((pDataParams->bAuthMode == PHAL_MFNTAG42XDNA_AUTHENTICATE) ? 0x02U : 0x04U);
    ...
    uint16_t wSDMMacKeyVer, uint8_t * pUid, uint8_t bUidLen, uint8_t * pSDMReadCtr, uint8_t * pInData, uint16_t wInDataLen, uint8_t * pRespMac)
{
...

    {
        ...
        if(pSDMReadCtr != NULL)
        {
            ...
            dwSDMctr |= (dwSDMctrtemp << 8U);
            dwSDMctrtemp = pSDMReadCtr[2];
            dwSDMctr |= (dwSDMctrtemp << 16U);
            dwSDMctrtemp = pSDMReadCtr[3];
            dwSDMctr |= (dwSDMctrtemp << 24U);
        }

        /* If SDM Ctr is 0xFFFFFF, then return error */
        if (dwSDMctr == 0xFFFFFFU)
        {
        ...
        if(bSdmOption & PHAL_MFNTAG42XDNA_RDCTR_PRESENT)
        {
            ...
            if(dwSDMctr != 0U)
            {
                bSV[bSVMacLen++] = (uint8_t)(dwSDMctr & 0xFFU);
                bSV[bSVMacLen++] = (uint8_t)((dwSDMctr >> 8U) & 0xFFU);
                bSV[bSVMacLen++] = (uint8_t)((dwSDMctr >> 16U) & 0xFFU);
            }
        }
