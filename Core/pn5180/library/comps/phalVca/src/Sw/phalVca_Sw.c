    ...
    uint8_t * pPubRespTime, uint8_t * pCumRndRC)
{
    ...
    if(statusTmp == PH_ERR_SUCCESS)
    {
        ...
        if((pDataParams->eVCState != VC_DF_AUTH_EV2) && (pDataParams->eVCState != VC_DF_AUTH_ECC) &&
            (pDataParams->eVCState != VC_DF_AUTH_AES) && (pDataParams->eVCState != VC_MFP_AUTH_AES_SL1) &&
            (pDataParams->eVCState != VC_MFP_AUTH_AES_SL3))
            pDataParams->eVCState = VC_PROXIMITYCHECK;
    }
    else
    ...
    uint16_t wRespLen, uint16_t wKeyNumber, uint16_t wKeyVersion, uint8_t * pRndCmdResp)
{
    ...
    uint8_t     PH_MEMLOC_REM bMacLength;
    uint8_t     PH_MEMLOC_REM bIv_bak[PH_CRYPTOSYM_AES128_KEY_SIZE];
    uint8_t     PH_MEMLOC_REM Key_bak[PH_CRYPTOSYM_AES256_KEY_SIZE];
    ...
    (void)memset(aKey, 0x00, PH_CRYPTOSYM_AES256_KEY_SIZE);
    ...
    statusTmp = phKeyStore_GetKey(pDataParams->pKeyStoreDataParams, wKeyNumber, wKeyVersion, PH_CRYPTOSYM_AES256_KEY_SIZE,
        aKey, &wKeyType);
    ...
    switch(pDataParams->eVCState)
    {
        ...
        case VC_PROXIMITYFAILED:
            /* Generating a random DummyMACKey of 32 bytes length */
            PH_CHECK_SUCCESS_FCT(statusTmp, phCryptoRng_Rnd(pDataParams->pCryptoRngDataParams,
                phKeyStore_GetKeySize(wKeyType), aKey));
            break;
        ...
        case VC_DF_AUTH_ISO:
            ...
            /* Verify the Status of GetKey */
            PH_CHECK_SUCCESS(statusTmp);

            /* Check key type */
            if((wKeyType != PH_CRYPTOSYM_KEY_TYPE_AES128) && (wKeyType != PH_CRYPTOSYM_KEY_TYPE_AES256))
            {
                return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_AL_VCA);
            }
            break;
        ...
        case VC_MFP_AUTH_AES_SL3:
            (void)memcpy(aKey, pDataParams->bSessionAuthMACKey, ((wKeyType != PH_CRYPTOSYM_KEY_TYPE_AES256)? PH_CRYPTOSYM_AES128_KEY_SIZE: PH_CRYPTOSYM_AES256_KEY_SIZE));
            break;
        default:
            break;
    }
    ...
    phhalVca_Sw_Int_BackupKey(pDataParams, Key_bak, wKeyType);
    ...
    status = phCryptoSym_CalculateMac(
        ...
        PH_CRYPTOSYM_MAC_MODE_CMAC,
        aTmpBuf,
        (uint16_t)(aTmpBufLen + (PHAL_VCA_PC_RND_LEN_NEW * 2U)),
        bMac,
        &bMacLength);
    if(status != PH_ERR_SUCCESS)
    {
    ...
    /* restore backup key */
    phhalVca_Sw_Int_RestoreKey(pDataParams, Key_bak, wKeyType);
...
phStatus_t phalVca_Sw_SetSessionKeyUtility(phalVca_Sw_DataParams_t * pDataParams, uint8_t * pSessionKey, uint8_t bAuthMode)
{
    ...
    case 0x71U:  /* EV2 Authenticate state for DESFire and DUOX PICC */
        ...
        (void)memcpy(pDataParams->bSessionAuthMACKey, pSessionKey, sizeof(pDataParams->bSessionAuthMACKey));
        break;
#endif /* defined(NXPBUILD__PHAL_MFDFEVX_SW) || defined(NXPBUILD__PHAL_MFDUOX_SW) */
#ifdef NXPBUILD__PHAL_MFDUOX_SW
    case 0x87U:  /* ECC Authenticate state for DUOX PICC */
        pDataParams->eVCState = VC_DF_AUTH_ECC;
    ...
    uint8_t * pRandChal, uint8_t * pVCData)
{
    phStatus_t  PH_MEMLOC_REM statusTmp;
    uint8_t     PH_MEMLOC_REM aKey[PH_CRYPTOSYM_AES128_KEY_SIZE];
    uint8_t     PH_MEMLOC_REM bInDataLen;
    uint16_t    PH_MEMLOC_REM wKeyType;
    uint8_t     PH_MEMLOC_REM bIv[16];
    uint8_t     PH_MEMLOC_REM bIv_bak[PH_CRYPTOSYM_AES128_KEY_SIZE];
    ...
    bInDataLen = (uint8_t) strlen((const char *) pInData);
...
void phhalVca_Sw_Int_BackupKey(phalVca_Sw_DataParams_t * pDataParams, uint8_t *pKey_Bak, uint16_t wKeyType)
{
    ...
    switch(PH_GET_COMPID(pDataParams->pCryptoDataParams))
    {
        ...
        case PH_CRYPTOSYM_SW_ID:
            ...
            if(pKey_Tmp != NULL)
            {
                (void) memcpy(pKey_Bak, pKey_Tmp, phKeyStore_GetKeySize(wKeyType));
            }
            break;
#endif /* NXPBUILD__PH_CRYPTOSYM_SW */
...
void phhalVca_Sw_Int_RestoreKey(phalVca_Sw_DataParams_t * pDataParams, uint8_t *pKey, uint16_t wKeyType)
{
    uint8_t * pKey_Tmp = NULL;
    ...
    switch(PH_GET_COMPID(pDataParams->pCryptoDataParams))
    {
        ...
        case PH_CRYPTOSYM_SW_ID:
            ...
            if(pKey_Tmp != NULL)
            {
                (void) memcpy(pKey_Tmp, pKey, phKeyStore_GetKeySize(wKeyType));
            }
            break;
#endif /* NXPBUILD__PH_CRYPTOSYM_SW */
